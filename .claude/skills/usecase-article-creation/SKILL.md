...
- 想定読者像とコンバージョン地点を明確化
- `docs/blog/research/` 配下にリサーチ資料があれば必読
...
| `body` | HTML 文字列（後述） |
| `image` | サムネイルパス（`/images/blog/<slug>/thumbnail.webp`）。**`image` は必須でフォールバックが無いため、実在しないパスを指すと一覧カード・記事ヒーロー・OG画像・Article JSON-LD がすべて404になる**（`tsc` も `next build` も通るのでCIでは検出できない）。画像が未作成なら公開しない。他記事のサムネイルを流用すると、焼き込み文字が記事内容と食い違うので使わない |
...
- フックを書く**前に** `docs/blog/guidelines/blog_opening_hooks.md` の管理表を読み、過去記事と題材・オチ・書き出しの構文が被らないようにする
- 記事完成後、同じ管理表に**必ず1行追記**する（記事ID・公開日・題材カテゴリ・オチの種類・書き出しの1文）
- 管理表の更新は Phase 15 のチェックリスト項目になっている。追記漏れのまま完了としない

### 記事内で使える組み込みコンポーネント

`components/news/NewsBody.tsx` の `sanitize-html` が許可している属性に対応した、
記事HTML内で使えるパターン。新しいパターンを足すときは sanitize の許可属性と
`app/globals.css` のスタイルもセットで更新する。

| 用途 | マークアップ | スタイル定義 |
|---|---|---|
| FAQ | `<section class="faq">` ＋ `<h3>`（問）→ `<p>`（答）の繰り返し | `app/globals.css` の `.prose section.faq` |
| 目次 | `<nav class="toc" aria-label="この記事の目次">` ＋ `<p>目次</p>` ＋ `<ol><li><a href="#id">` | `app/globals.css` の `.prose nav.toc` |
| 見出しアンカー | `<h2 id="kebab-case">`（`h3` も可） | `[id] { scroll-margin-top }` が固定ヘッダー分を吸収 |

目次を入れる場合は、`<a href="#...">` と `<h2 id="...">` を手で対応させる必要がある。
記事完成後に件数・id・**並び順**の一致を必ず確認すること（本文の h2 出現順と目次の順が違うと読者が迷う）。

#### 見出し階層の設計（目次の長さは h2 の数で決まる）

目次は `h2` だけを載せる。**`h2` は 7〜9 個に収める。** 10 個を超えると目次だけで
1 画面が埋まり、記事全体を見渡すという目次本来の役割を果たさなくなる。

`h2` に置くのは「読者が読むかどうかを決める単位」だけにする。判断基準は次のとおり。

| 置き場所 | 内容 |
|---|---|
| `h2` | 読者の意思決定の単位（〜とは／進め方／向かないケース／選び方／まとめ／FAQ） |
| `h3` | その中の手順・具体例・補足・理由の説明 |

h2 が増えすぎたときは、隣り合う h2 をまとめて 1 つの h2 にし、元の見出しを h3 に落とす。
このとき**元の h2 が持っていた h3 は h3 のまま兄弟にする**（`h4` を作らない。`h4` は
prose のスタイル定義がなく、目次からも辿れない）。

降格・統合したら次を必ず直す。

- 目次の `<li>` を削る（本文の順序と一致させる）
- 他記事・他ページから消えたアンカー（`/usecases/<slug>#<id>`）へのリンクが無いか grep する

### 見出しの装飾（`app/globals.css` の `.prose h2`）

`h2` は上に forest 2px、下にヘアラインを引いてセクションの切れ目を示す。
`nav.toc` と同じ「罫線と余白で構造を作る」語彙に揃えてあり、
カード・角丸・背景色・左のアクセントバーは使わない。記事側の対応は不要。
...
`content/usecases/index.ts` の `USECASES_ARTICLES` 配列に追加する。`import` 文も忘れずに。

表示順は配列末尾の `.sort()` が更新日の降順で決める。**同じ日付のときは配列に書いた順がそのまま表示順になる**ので、同日公開の記事が複数あるときは配列の並びで優先度を表現する。可読性のため、import 順・配列順・実際の表示順は揃えておく。

`USECASES_CARDS`（body を落としたカード用の射影）も同ファイルが export している。**client component からは `@/content/usecases` を import しない**こと。import すると全記事の本文HTMLがブラウザのJSバンドルに載る。カルーセル等へは server component 側で `USECASES_CARDS` を props で渡す（`app/page.tsx` → `components/home/CasesCarouselSection.tsx` が実例）。
...
- テーブルは `NewsBody.tsx` が `.table-scroll` で囲む。外枠（角丸ボーダー）・セル余白・ヘッダー背景・列区切り・最小幅 30rem・横スクロールは `app/globals.css` 側で完結しているので、記事側の対応は不要
...
理由：見出しの直後に図があると、読み手はそのセクションで何を扱うのかを読む前に把握できる。
本文を読み終えた後ろに図があると、同じ内容を二度たどることになり認知的負荷が上がる。

- 文章の途中や、セクションの末尾に置かない
- 1 セクションに図は原則 1 枚。複数ある場合はセクションを分けるか、`<h3>` の直後に置く
