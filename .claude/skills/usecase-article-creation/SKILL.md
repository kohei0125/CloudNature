...
- テーブルは `NewsBody.tsx` が `.table-scroll-wrap > .table-scroll` で囲む。外枠（角丸ボーダー）・セル余白・ヘッダー背景・列区切り・最小幅 30rem・横スクロールは `app/globals.css` 側で完結しているので、記事側の対応は不要
- 横に溢れる表には「横にスライドできます」が自動で付く（`components/news/TableScrollHints.tsx` が実測して出し分ける）。記事側で注記を書き足さない
...
### 7.6 `NewsBody.tsx` の prose 設定（現行値）
