...
const HeroCardMetrics = () => {
  return (
    <div className="h-[260px] md:h-[220px] lg:h-full rounded-[24px] md:rounded-[40px] glass-card-dark p-4 md:p-6 v-stack justify-center relative overflow-hidden group shadow-2xl texture-grain border border-white/5 w-full">
      ...
      {/* 3D Icon Image */}
      <TrendingUp className="w-8 h-8 text-sunset mb-4 relative z-10" />
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-baseline gap-1">
          <span className="text-3xl md:text-5xl font-bold text-white highlight-cloud">
            {HERO_BENTO.metricCard.value}
          </span>
          <span className="text-base md:text-xl text-sage font-bold">
            {HERO_BENTO.metricCard.unit}
          </span>
        </div>
        <p className="text-gray-400 text-[10px] md:text-xs mt-1 md:mt-2 leading-tight whitespace-pre-line">
          {HERO_BENTO.metricCard.description}
        </p>
      </div>
    </div>
  );
