    ...
    <section id="hero" data-hero-section className="relative overflow-hidden bg-white">
      ...
      >
        {/* 写真の下端（川）を切り落として橋を下寄りに見せるため、
            ...
            この構成では画像の高さ方向は全部入るので、ボックスを高くするほど
            被写体が下へ送られて川が切れる。133% で橋脚の水際が画面の約 93% に来る
            （橋の欄干は画像の 65.5%、水際は 69.7% の位置） */}
        <div className="animate-hero-zoom-in absolute inset-x-0 top-0 h-[133%]"
        ...
        {/* テキスト帯を最も白くし、上は空が薄く透ける程度、下は写真をそのまま見せる。
            ...
            テキストは 21%〜63% に収まるので、白は 63% までフルで保ち、そこから一気に抜く。
            上の 133% 敷き込みを前提に、ビル群は約 76%、橋は約 87%、水際は約 93% に来るため、
            85% 以降をほぼ素の写真にして橋の色が飛ばないようにしている。
            ボックスの高さを変えたらこのストップ位置も合わせて引き直すこと */}
        <div
          aria-hidden
          ...
          style={{
            background: [
              ...
              "rgba(255,255,255,0.72) 69%",
              "rgba(255,255,255,0.44) 75%",
              "rgba(255,255,255,0.20) 81%",
