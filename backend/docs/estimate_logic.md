...
┌─────────────────────────────────────────────────────────┐
│ 0. Idempotency チェック                                   │
│    └─ 同一session_idでstatus=completedのレコードが         │
│       存在する場合、キャッシュ済みJSON結果を即時返却         │
│       （LLM再生成・メール重複送信を防止）                   │
└────────────────────┬────────────────────────────────────┘
                     ▼
┌─────────────────────────────────────────────────────────┐
...
未知のカテゴリはデフォルト「基本データ管理」にフォールバックする。フォールバック発生時は `WARNING` レベルのログを出力し、対象のfeature valueとデフォルトカテゴリを記録する（観測性向上）。
...
**複雑カテゴリ抑制:** 以下のカテゴリに該当する機能が2つ以上含まれる場合、ディスカウントは最低0.90に抑制される（0.78にはならない）:
...
```json
{
  ...
  "features": [
    { "name": "機能名", "detail": "説明（30文字以内）", "standard_price": 0, "hybrid_price": 0 }
  ],
...
| # | チェック | 失敗条件 |
|---|---------|---------|
| 1 | JSONスキーマ検証 | `step8_features` が配列でない、3〜6個でない、必須フィールド欠落 |
...
│ /generate 重複リクエスト          │ completed済みならキャッシュ返却  │
└──────────────────────────────────┴──────────────────────────────┘
```
