...
export const PAGE_META = {
  // トップは会社全体の傘。H1・title は「AI開発・AI導入支援パートナー」軸で統一し、
  // 本文のAIトランスフォーメーション訴求とブランドを揃える。
  // 判断の経緯は docs/adr/0003-restore-ai-axis-on-top-page.md を参照（ADR 0002 をトップの見出し軸のみ再度supersede）。
  home: {
    title: "新潟のAI開発・AI導入支援パートナー｜クラウドネイチャー",
    description:
      "新潟市を拠点に、AIエージェント開発・AI導入支援・業務システム開発を手がける会社です。受発注・在庫・勤怠のオーダーメイド構築から、既存システムとの連携、判断を伴う業務の自動化まで。小さく始めて、使われる状態になるまで伴走します。"
  },
  services: {
    title: "新潟のAIエージェント開発・システム開発・AI導入支援 | 株式会社クラウドネイチャー",
    description: "AIエージェント開発、業務システム開発、AI導入・伴走支援の3つのサービスで、新潟の企業の業務効率化・人手不足解消をスモールスタートで実現。開発費用の概算がその場で分かる無料のAI見積もりもご利用いただけます。"
  },
  // /cases は「顧客導入事例」ではなく自社の開発実績ページ（2026-08-27 改定）。
  // 「見積もりai 中小企業」等で1ページ目に到達しているため title からキーワードは落とさず、
  // 「自社開発」を明示して実態と表示を揃える。docs/20260827_cases_page_restructure_review.md 参照。
  cases: {
    title: "AI開発・活用事例｜自社開発のAI見積もりで工数90%削減 | クラウドネイチャー",
    description: "新潟のAI開発会社クラウドネイチャーが、自社の業務課題のために開発したAI・システムの実績です。見積もり作成を1件あたり約1時間から数分へ短縮したAI見積もりシステムは、稼働中のものをそのまま公開しています。"
  },
  contact: {
  ...
  company: {
    ...
    description: "株式会社クラウドネイチャーの会社概要・代表メッセージ・アクセス情報。AI導入支援・AIエージェント開発・業務システム開発の3つのサービスで、新潟の企業に寄り添う伴走型パートナーです。"
  },
  security: {
  ...
  servicesAiSupport: {
    ...
    description: "「何から始めればいいか分からない」を解決。無料診断で自動化できる業務を特定し、AI導入から運用定着まで伴走。経営者向けAIスクール・法人研修も提供。新潟の企業のAI業務効率化を支援します。"
  },
  servicesAiAgent: {
    title: "新潟のAIエージェント開発・チャットボット構築 | 株式会社クラウドネイチャー",
    description: "社内ナレッジAI、問い合わせ自動応答、SaaS間データ連携など、24時間365日稼働するAIエージェントを開発。LINE・Slack・kintone連携対応。新潟の企業の人手不足を根本から解決します。"
  },
