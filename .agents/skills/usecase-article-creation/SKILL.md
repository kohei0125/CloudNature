...
**目次（`nav.toc`）より前に置いてよいのは、冒頭フックと 1〜2 文のリードだけ。**
合計で**全角 400 字前後、多くても 500 字**、段落は 7 つ以内に収める。

読者は目次を見て「読むかどうか」を決める。そこへ辿り着くまでが長いほど、
本文に入る前に離脱する。スマホでは 500 字が 1 画面を超える。

超えたときは、たいてい**本文に属する内容が冒頭に混ざっている**のが原因で、
フックそのものが長いわけではない。次のものは冒頭に置かず、最初の `<h2>` へ移す。

| 冒頭に混ざりがちなもの | 本来の置き場所 |
|---|---|
| 用語の定義（「この記事では〇〇を次のように定義します」） | 「〜とは」の `h2` |
| 前提の説明・記事のスコープ宣言 | 「〜とは」の `h2`、または該当セクション |
| 他社サービス・競合の紹介 | 比較・使い分けの `h2` |
| サービスページへの内部リンク | 関連する本文セクション |

「この記事で得られるもの」を書く場合も 1〜2 文にとどめる。目次が直後にあるので、
扱う内容の列挙は目次が代わりに果たしている。

機械的に確認する：

```bash
# body 先頭から <nav class="toc"> までの文字数（タグ・空白を除く）
python3 - content/usecases/<slug>.ts <<'PY'
import re,sys
s=open(sys.argv[1],encoding='utf-8').read()
m=re.search(r'body: `(.*?)<nav class="toc"', s, re.S)
print(len(re.sub(r'\s','',re.sub(r'<[^>]*>','',m.group(1)))) if m else 'nav.toc なし')
PY
```

既存記事の実測値（2026-08 時点）は 284〜414 字。この幅に収まっていれば適正。
...
- 手順・構成・分岐・対応関係は図解セクションに置き換える（下記）

### 7.5 図解セクション（文章を図に置き換える）

手順・構成・分岐・対応関係は、文章で説明すると読み手が頭の中で図を組み立て直すことになる。
`app/globals.css` に用意した図解クラスで、同じ内容を形のまま見せる。

**制約**：`NewsBody.tsx` の sanitize は `class` を `section` / `blockquote` / `nav` にしか許可しない。
図解は必ず `<section class="...">` を器にして、中身は要素の並びだけで組む（`div` / `span` に `class` は付けられない）。

| クラス | 形 | 使いどころ | 中身の構造 |
|---|---|---|---|
| `flow` | 横並びの手順（01 → 02 → 03） | 3ステップの流れ | `p`（ラベル）＋ `ol > li > strong + span` |
| `stack` | 上から下へ流れる構成 | 全体の見取り図、STEP の流れ | `p` ＋ `ol > li > strong + span`（`li > ul > li` で入れ子可） |
| `branch` | 入口 → 2つの経路 → 合流 | 条件分岐 | `p`（入口）＋ `ul > li > strong + span` ＋ `p`（合流） |
| `roles` | 主従のあるカード2枚 | 誰が主体かを示す（1枚目が主体） | `p` ＋ `ul > li > em + strong + span` |
| `rename` | 旧 → 新の対応（列見出し付き） | 名称変更・統合 | `p > em + em`（列見出し）＋ `ol > li > (strong \| ul) + span > small` |

**判断基準**：既存の図解画像とテーブルで足りているところに図解を足さない。二重になって逆に重くなる。
文章だけで説明している「手順・分岐・対応関係」を探して置き換える。
...
bash .Codex/skills/usecase-article-creation/scripts/remove_wm_clone.sh \
...
```bash
node .Codex/skills/usecase-article-creation/scripts/screenshot.cjs \
...
これは CloudNature プロジェクト全体のルール（AGENTS.md）として、リファクタリング・ロジック変更・バグ修正などの軽微でない作業の最後に必ず実行することになっている。`/simplify` スキルは以下を点検する：
...
□ body 先頭から目次までが全角 500 字以内（定義・前提・競合紹介を冒頭に置いていない）
