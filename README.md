...
NOTION_API_KEY=            # 任意。Notion「案件テーブル」への自動保存に使用
NOTION_DATABASE_ID=        # 保存先: 登壇・セミナー管理ページの案件テーブル
...
- **Notion連携**: `NOTION_API_KEY` / `NOTION_DATABASE_ID` 設定時、メール送信後に Notion「登壇・セミナー」ページの「案件テーブル」へ自動で1件追加する（`notionService.ts`）。未設定時は保存をスキップしフォーム自体は正常動作する。本体コーポレートサイトの問い合わせ保存（`app/api/contact/notionService.ts`）と同じ `after()` パターンを採用（レスポンス返却後も保存処理を完走させるため）
