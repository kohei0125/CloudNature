...
## 7-2. 追加対応: メインビジュアルの「ズレ」（2026-08-17）

### 報告

「リロードすると、メインビジュアルの位置が最初の1秒弱くらいずれる。
数秒してテキストが表示されると元に戻る」
...
- `layout-shift` エントリは dev・本番ビルドとも **0件**（`PerformanceObserver` の `buffered: true` で確認）
- ヒーローの高さは常に **712px**（`min-height: clamp(560px, 182vw, 820px)` = 711.62px が支配）
- テキストブロックは 299px + `padding-bottom` 113.86px = 412.86px で `min-height` を超えないため、
  **テキストの表示・非表示で高さは構造上変わらない**
- CSS 未適用（FOUC相当）を再現しても、逆に高くなる（712px → 1321px）ため症状と一致しない

### 原因

背景画像に掛かっていた `hero-zoom-in` が `scale(1.03) → none` を 1200ms かけて実行しており、
**メインビジュアルが 1.2 秒かけて縮みながら現れていた**。レイアウトは動かないため計測には
出ないが、視覚的には「絵がずれている」と受け取られる。報告の「最初の1秒弱」と時間が一致する。

### 対応

`hero-zoom-in` から `transform` を外し、opacity のみのフェードに変更。
ズームしなくなったため名称も実態に合わせて **`hero-image-in`** へ改名した
（`tailwind.config.ts` と `components/home/HeroSection.tsx` の2箇所）。

検証（Web Animations API でシーク）:

| currentTime | transform | opacity | 位置/高さ |
| --- | --- | --- | --- |
| 0ms | none | 0 | 0 / 890 |
| 300ms | none | 0.765 | 0 / 890 |
| 600ms | none | 0.961 | 0 / 890 |
| 1200ms | none | 1 | 0 / 890 |

全区間で `transform: none`、位置・サイズとも不変。ヒーロー高も 712px のまま。

### 未確認

いただいたスクリーンショットでは、ヒーローが 712px より低く見え、その下の NEWS が
同一画面内に入っていた。この状態は再現できていない（撮影時のウィンドウ幅の可能性）。
本対応後も同じ見え方が残る場合は、ブラウザ・ウィンドウサイズの情報が必要。
