...
## 追記（2026-07-29 その9）: モバイルの改行制御・会社概要見出しの左揃え

- **タイトルの改行制御**: OUR PURPOSE がモバイルで「〜ものにし／ない」と不自然に折り返していたため、
  タイトルを `\n` 区切りセグメントの `inline-block` 描画に変更（折り返しは区切り位置でのみ発生、
  収まる幅では1行のまま）。ヒーローの「AIを、／経営のど真ん中へ」にも同方式を予防適用。
  今後、長い日本語タイトルを追加する際はこのパターンを使うこと。
- **会社概要見出しの左揃え**: 共有 `SectionHeader`（非センター版）はモバイルで中央寄せになる仕様のため、
  左揃えの他セクションの中で会社概要だけ右に寄って見えていた。他の企業情報セクションと同じ
  手組みヘッダー（常に左揃え、title は text-forest）に変更し、`SectionHeader` 依存を外した。
- 検証: build / ESLint パス。モバイル500px・PC1600px でタイトル1行維持と左揃えを確認。
