...
| 対策 | 効果 | 難易度 |
|------|------|-------|
| **A. AI動的質問プロンプトにstep_7=enhancement時の機能数上限（3〜4件）を追加** | スパム対策等の軽微案件で過剰提案を抑制 | 低 |
| **B. 機能数ディスカウントのカーブをさらに強化（7件→0.80等）** | 7機能でも金額を抑制 | 低 |
| **C. AIに「PoC/MVP」フラグを判定させ、段階に応じた機能数の制限を行う** | PoC案件の過剰見積もりを防止 | 中 |
| **D. 「カスタム開発対象外」判定ロジックの追加** | iPaaS/CMS案件を自動的に低コスト提案に振り分け | 高 |
