...
| 3b | **P0** | `--` | `[外]` | 完了 | **GA4・Search Console 事前確認** | データストリーム追加（ai: G-BKHWKEZ26E）、SC両サイト登録済み、インデックス済み、robots.txt OK |
...
1. GA4管理画面 → **プロパティ設定 → データストリーム**
   - cloudnature.jp と ai.cloudnature.jp の両方がストリームとして存在するか確認
   - もし1つしかなければ追加（サブドメインは別ストリームが必要な場合がある）
2. GA4 → **リアルタイムレポート**
   - cloudnature.jp にアクセス → アクティブユーザー「1」が表示されるか
   - ai.cloudnature.jp にアクセス → 同様に表示されるか
   - **見積もりサイト側で表示されない場合** → Vercel環境変数に `NEXT_PUBLIC_GA_ID` が設定されているか確認
